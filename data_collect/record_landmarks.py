...
Chirality
---------
Landmarks are extracted from the UNMIRRORED camera frame, matching how the
ASL/DGS data is extracted (dataset_builder/frankenstein_builder.py) and how the
served app sees the webcam. Only the on-screen preview is mirrored, so it still
behaves like a mirror for the person recording.

Any .npy files recorded before this change were captured from a mirrored frame
and therefore describe the opposite hand. Re-record them, or mirror them once
with:
    a = np.load(path).reshape(-1, 21, 3); a[:, :, 0] = -a[:, :, 0]
(negating x is a true mirror once the landmarks are wrist-centered downstream).
"""
...
class LandmarkRecorder:
    ...
    def run(self):
        ...
        while True:
            ...
            # Detect on the UNMIRRORED frame so the recorded hand matches the
            # rest of the dataset and the served app. The preview is flipped
            # further down, after the overlay is drawn.
            ...
            # Mirror for display only. Done after the landmark overlay so it
            # stays aligned, and before the text so the text reads normally.
