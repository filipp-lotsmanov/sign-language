...
from src.backend.core.config import ALL_LETTERS, STATIC_LETTERS
...
class LetterSequence:
    ...
    def _get_random(self, current_letter: Optional[str]) -> str:
        ...
        candidates = [letter for letter in self.available_letters if letter != current_letter]
        if not candidates:
            candidates = self.available_letters
        choice = random.choice(candidates)
        logger.debug(
            "Random mode: selected '%s' from %d candidates (avoiding '%s')",
            choice,
            len(candidates),
            current_letter,
        )
