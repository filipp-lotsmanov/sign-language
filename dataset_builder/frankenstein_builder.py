...
from paths import FRANKENSTEIN_DIR, HAND_LANDMARKER  # noqa: E402
...
# MediaPipe model path (shared with the serving path via paths.py)
...
class FrankensteinBuilder:
    def __init__(
        ...
        asl_images_path: str,
        dgs_csv_path: str,
        # Stage 1 of the pipeline. Defaults to the shared location so that
        # augment_landmarks.py picks the output up without reconfiguration.
        output_path: str = str(FRANKENSTEIN_DIR),
        ...
        for letter, source in LETTER_SOURCE.items():
            ...
            if source == "asl":
                ...
                if len(landmarks) == 0:
                    ...
                    print(" DGS empty, fallback to ASL")
                    landmarks = self.extract_asl_landmarks(letter, self.samples_per_class)
